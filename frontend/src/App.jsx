...
function App() {
  return <BrowserRouter><Routes><Route path="/login" element={<Login />} /><Route path="/register" element={<Register />} /><Route element={<ProtectedRoute />}><Route element={<AppShell />}><Route path="/dashboard" element={<Dashboard />} /><Route path="/snippets" element={<MySnippets />} /><Route path="/search" element={<SearchResults />} /><Route path="/snippets/new" element={<SnippetForm />} /><Route path="/snippets/:id" element={<SnippetDetails />} /><Route path="/snippets/:id/edit" element={<SnippetForm editing />} /></Route></Route><Route path="*" element={<Login />} /></Routes></BrowserRouter>
}

function AppShell() { return <><header className="topbar"><Link className="brand" to="/snippets">CodeVault</Link><SearchBar /><nav><Link to="/snippets">My snippets</Link><Link to="/dashboard">Dashboard</Link></nav></header><Outlet /></> }
